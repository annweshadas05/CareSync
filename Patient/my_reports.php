...
$pstmt->execute();
$raw_prescriptions = $pstmt->get_result()->fetch_all(MYSQLI_ASSOC);
$pstmt->close();

// Group medicines by their prescription timestamp so all medicines
// issued at the same time appear as a single prescription card.
$prescriptions = [];
foreach ($raw_prescriptions as $row) {
    $key = $row['created_at']; // use timestamp as grouping key
...
// Re-index for cleaner iteration
...
<div class="sidebar shadow">
    ...
    <nav class="nav flex-column">
        ...
        <a class="nav-link active" href="my_reports.php"><i data-lucide="activity"></i> <span>My Reports</span></a>
        ...
        <?php else: ?>
            ...
            <div class="presc-card">
                <div class="presc-header">
                    <span class="fw-bold" style="color:#061727;">
                        <i data-lucide="file-text" width="16" class="me-1"></i>
                        Prescription
                        <small class="text-muted ms-2 fw-normal">
                            (<?= count($presc['medicines']) ?> medicine<?= count($presc['medicines']) > 1 ? 's' : '' ?>)
                        </small>
                    </span>
                    <span class="date-badge" style="background:var(--blue-gradient);color:#fff;border-radius:8px;padding:4px 12px;font-size:0.8rem;font-weight:800;">
                        <?= date('d M Y, h:i A', strtotime($presc['created_at'])) ?>
                    </span>
                </div>
                ...
                <div class="p-3">
                    <table class="presc-table w-100">
                        <thead>
                            <tr>
                                <th>#</th><th>Medicine</th><th>Form</th><th>Dosage</th>
                                <th>Frequency</th><th>Duration</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($presc['medicines'] as $idx => $med): ?>
                            <tr>
                                <td class="fw-bold text-muted"><?= $idx + 1 ?></td>
                                <td class="fw-bold text-dark"><?= htmlspecialchars($med['medicine'] ?? '—') ?></td>
                                <td class="fw-bold text-dark"><?= htmlspecialchars($med['form'] ?? '—') ?></td>
                                <td class="fw-bold text-dark"><?= htmlspecialchars($med['dosage'] ?? '—') ?></td>
                                <td class="fw-bold text-dark"><?= htmlspecialchars($med['frequency'] ?? '—') ?></td>
                                <td class="fw-bold text-dark"><?= htmlspecialchars($med['duration'] ?? '—') ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
